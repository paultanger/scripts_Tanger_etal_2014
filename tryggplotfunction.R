...
#library(ggplot2)
mydotplot = function(mydata, mytitle, myxlab, myylab, fontsize=12, pointsize=4, errbarht=.4){
  dotplot <- ggplot(mydata, aes(x=mean, y= line)) + # try removing this: , fill= enviro, color=enviro
  #  geom_dotplot(binaxis = "y", stackgroups = FALSE, binpositions="all", stackdir="center", dotsize=2) # try commenting out this line
  # try adding this
  ...
  # adjust scale
  #dotplot <- dotplot + scale_x_continuous(limits= c(0, max(nick.summary.data$mean)+5), breaks = round((seq(0, max(nick.summary.data$mean)+5, by = 5)/5)) * 5)
